        ...
        <tbody>
          @foreach($record->sellingDetails as $detail)
            <tr>
              <td class="p-2 border">
                @if($detail->product)
                  <h3 class="text-primary">{{ $detail->product->name }}</h3>
                @else
                  <h3 class="text-primary text-red-500">@lang('Product not found')</h3>
                @endif
    ...
    printButton.addEventListener('click', async () => {
      ...
      try {
        if (!printerData) {
          ...
          // Handle null user
          if (selling.user && selling.user.name) {
          ...
          // Handle null payment_method
          if (selling.payment_method && selling.payment_method.name) {
            printerAction.table(['@lang('Payment method')', selling.payment_method.name]);
          }
          
          if(selling.member != undefined && selling.member != null) {
            printerAction.table(['Member', selling.member.name]);
          }
          ...
          selling.selling_details.forEach(sellingDetail => {
            // Handle null product
            if (sellingDetail.product && sellingDetail.product.name) {
              ...
              printerAction.table(['@lang('Product not found')', '-'])
            }
          });
