...
@script()
  ...
  return {
    ...
    // Inisialisasi saat component dimuat
    init() {
      // Set focus pada input display
      this.$nextTick(() => {
        if (!this.isTouchScreen()) {
          this.$refs.payedMoney.focus();
        }
      });
      
      // Tambahkan event listener untuk keyboard input
      this.$refs.payedMoney.addEventListener('input', (e) => {
    ...
    handleKeyboardInput(event) {
      // Ambil value dan hapus semua karakter kecuali angka dan titik
      ...
      event.target.value = moneyFormat(rawValue);
    ...
    append(number) {
      ...
      // Jika displayValue adalah 0, ganti dengan angka baru
    ...
    changes() {
      // Parse nilai yang sudah dibersihkan
      let num = parseFloat(this.displayValue.replace(/,/g, ''));
      num = isNaN(num) ? 0 : num;
      
      // Hitung kembalian
      let moneyChanges = num - this.subtotal;
      
      // Update wire
      $wire.cartDetail['money_changes'] = moneyChanges;
      $wire.cartDetail['payed_money'] = num;
      
      // Update tampilan kembalian
      if (this.$refs.moneyChanges) {
        this.$refs.moneyChanges.textContent = moneyFormat(moneyChanges);
      }
    }
  }
});
