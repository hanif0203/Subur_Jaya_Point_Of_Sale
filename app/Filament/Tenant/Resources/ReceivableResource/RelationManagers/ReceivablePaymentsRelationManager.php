...
class ReceivablePaymentsRelationManager extends RelationManager
{
    ...
    {
        return can('lihat piutang') || can('read receivable payment');
    }
        ...
        return $table
            ...
            ->headerActions([
                Tables\Actions\CreateAction::make()
                    ...
                    ->visible(fn () => can('create receivable payment') && !$this->ownerRecord->status)
                    ->action(function (array $data): void {
                        $this->dPService->create($this->ownerRecord, $data);
                        $this->refreshPage();
                    }),
            ])
            ->actions([
                Tables\Actions\EditAction::make()
                    ->visible(fn () => can('update receivable payment'))
                    ->action(function (array $data, ReceivablePayment $receivablePayment): void {
                        $this->dPService->update($receivablePayment, $data);
                        $this->refreshPage();
                    }),
                Tables\Actions\DeleteAction::make()
                    ->visible(fn () => can('delete receivable payment'))
